...
def aspheric_surface_jacobian_equation(r, d, k, radius_of_curvature):
    """
    Representation of the aspheric surface function.
    """
    sample_number = r.size

    l = np.sqrt(np.negative(((k + 1) * np.power(r, 2) - radius_of_curvature * radius_of_curvature) / (
            radius_of_curvature * radius_of_curvature)))
    ...
    part_div_k = np.divide(num, den)

    # Partial derivative on the radius of curvature
    num = np.negative(np.power(r, 2))
    den = radius_of_curvature * radius_of_curvature * np.multiply(l, l + 1)
...
class Mirror:
    ...
    def find_best_fit_conic_on_offset_data(self):
        """
    ...
    def find_best_fit_conic_on_integrated_data(self):
        """
        Find the best fit conic for the given test data and desired mirror parameters.
        """
        ...
        best_fit_parameters, estimated_covariance = curve_fit(f=aspheric_surface_equation,
                                                              ...
                                                              ydata=mirror_profile,
                                                              p0=p0,
                                                              # sigma=self.test_measurement_data_f_standard_deviation,
                                                              # absolute_sigma=True,
                                                              check_finite=True,
                                                              method="trf",
                                                              jac=aspheric_surface_jacobian_equation,
                                                              x_scale=p_scale,  # "jac",
                                                              loss="soft_l1",
                                                              verbose=2)
    ...
    def integrate_mirror_test_data(self):
        """
        Find the best fit conic for the given test data and desired mirror parameters.
        """
        f_plus_radius_of_curvature = self.test_measurement_data_f + self.mirror_details['expected_radius_of_curvature']
    ...
    def generate_best_fit_conic_offset_samples(self):
        ...
        best_fit_offset = self.mirror_details['best_fit_radius_of_curvature'] + self.mirror_details['best_fit_d'] - \
                          self.mirror_details['expected_radius_of_curvature']

        return r_sample_points, \
               f_sample_points - self.mirror_details['best_fit_radius_of_curvature'] - self.mirror_details[
                   'best_fit_d'] + best_fit_offset
